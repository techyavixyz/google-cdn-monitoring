...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const [entries, nextPageRequest] = await logging.getEntries({
      filter,
      pageSize: parseInt(pageSize),
      pageToken
    });

    const nextPageToken = nextPageRequest?.pageToken || null;
    ...
    for (const entry of entries) {
      ...
      const requestUrl =
        ...
        payload.requestUrlPath ||
        httpRequest.requestUrlPath ||
        payload.requestPath ||
        payload.request?.url ||
        'unknown';
    ...
    const topIps = Object.entries(ipMap)
      ...
      .map(([ip, count]) => {
        ...
        const urlCounts = urlMapPerIp[ip] || {};
        const [topUrl = 'unknown'] =
          Object.entries(urlCounts).sort((a, b) => b[1] - a[1])[0] || [];
